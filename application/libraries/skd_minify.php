...
class skd_minify
{
	 ...
	 * @var string
	 */
	public $assets_dir = 'views/theme-store/assets';
		...
		if ($fh = fopen($out_file, 'w'))
		{
			foreach ($file_array as $file) {
				...
				if(have_posts($file['path'])) {
					...
					foreach ($file['path'] as $folder => $path_folder) {
						...
						else $contents = str_replace('../'.$folder, $path_folder.'/'.$folder, $contents);
					}
				}
			...
			if($type == 'css') {
				if(function_exists('theme_custom_css')) {
					fwrite($fh, theme_custom_css(false));
				}
			}

			if($type == 'js') {
				if(function_exists('theme_custom_script')) {
