...
class CI_Cart {
	...
	var $product_name_rules	= '\@\]\[\\\`\'\~\!\#\$\^\&\*\+\"\,\.\:\%\(\)\-_ a-z0-9ấầẩẫậẤẦẨẪẬắằẳẵặẮẰẲẴẶáàảãạâăÁÀẢÃẠÂĂếềểễệẾỀỂỄỆéèẻẽẹêÉÈẺẼẸÊíìỉĩịÍÌỈĨỊốồổỗộỐỒỔÔỘớờởỡợỚỜỞỠỢóòỏõọôơÓÒỎÕỌÔƠứừửữựỨỪỬỮỰúùủũụưÚÙỦŨỤƯýỳỷỹỵÝỲỶỸỴđĐ'; // alpha-numeric, dashes, underscores, colons or periods
	...
	public function __construct($params = array())
	{
		...
		if(isset($_SESSION['cart_contents']) && $_SESSION['cart_contents'] !== FALSE) {
	...
	function _insert($items = array())
	{
		...
		$items['name'] = removeHtmlTags($items['name']);
		...
		// Trim any leading zeros
		// $items['price'] = trim(preg_replace('/(^[-1]+)/i', '', $items['price']));
