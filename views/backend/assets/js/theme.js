...
$(function(){
    //active theme
    body.on('click', '.theme-active', function(event) {
		var $key = $(this).val();

		data = {
			'action' 	: 'ajax_theme_active',
			'value' 	: $key,
		}
    ...
    body.on('click', '.theme-info', function(event) {
		var $key = $(this).val();

		data = {
			'action' 	: 'ajax_theme_info',
			'value' 	: $key,
		}
		$jqxhr   = $.post(base+'/ajax', data, function(data) {}, 'json');
		$jqxhr.done(function( data ) {
	...
	body.on('click', '#item-data-save', function(event) {
    	...
    	datas.action = 'ajax_theme_option_save';
	...
	THEME EDITOR
	================================================================*/
	body.on('click', '#result-editor .nav-tabs li a[role="tab"]', function(event) {
		id 		= $(this).attr('href');
		id 		= id.substr(1);
		path 	= $(this).attr('path');
	});
		...
		$.get(base+'/theme/editor_scan', function(data) {
			...
			fileList.on('click', 'li.files', function(e){
				...
				if( $('#result-editor').find('.nav-tabs li a[href="#'+id+'"]').length == 0 ) {
					...
					edit = {
						'action' : 'ajax_theme_editor',
						'path'	 : path,
					}
					$jqxhr   = $.post(base+'/ajax', edit, function() {}, 'json');
